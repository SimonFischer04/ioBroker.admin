...
class ConfigPanel extends ConfigGeneric {
    ...
    render() {
        ...
        if (hidden) {
            if (schema.hideOnlyControl) {
                const item = (React.createElement(Grid2, { size: {
                    ...
                    }, sx: Utils.getStyle(this.props.oContext.theme, { marginBottom: 0, textAlign: 'left' /* marginRight: 8, */ }, schemaStyle, this.props.oContext.themeType === 'dark' && schema.darkStyle) }));
                if (schema.newLine) {
        ...
        if (this.props.custom) {
            return (React.createElement(Grid2, { key: `${this.props.attr}_${this.props.index}`, container: true, style: styles.fullWidth, columnSpacing: 2, rowSpacing: 1, sx: schemaStyle }, this.renderItems(items, disabled)));
        }
        let content;
        if (schema.collapsable) {
            content = (React.createElement(Accordion, { key: `${this.props.attr}_${this.props.index}`, style: styles.fullWidth, expanded: !!this.state.expanded, onChange: () => {
                ...
                React.createElement(AccordionSummary, { expandIcon: React.createElement(ExpandMoreIcon, null), sx: Utils.getStyle(this.props.oContext.theme, schemaStyle, this.props.oContext.themeType && schema.darkStyle, schema.color === 'primary'
                        ? styles.primary
                        : schema.color === 'secondary' && styles.secondary, { width: '100%' }) },
                    React.createElement(Typography, { style: styles.heading }, this.getText(schema.label))),
                React.createElement(AccordionDetails, null,
                    React.createElement(Grid2, { container: true, columnSpacing: 2, rowSpacing: 1, sx: { ...schemaStyle, width: '100%', padding: '10px' } }, this.renderItems(items, disabled)))));
        }
        else {
            const sx = Utils.getStyle(this.props.oContext.theme, this.props.style, schemaStyle, { width: '100%' }, this.props.isParentTab && styles.paper, this.props.isParentTab &&
            ...
            content = (React.createElement(Box, { component: "div", key: `${this.props.attr}_${this.props.index}`, className: this.props.className, sx: sx },
                React.createElement(Grid2, { container: true, columnSpacing: 2, rowSpacing: 1, sx: Utils.getStyle(this.props.oContext.theme, { width: '100%' }, this.props.isParentTab && styles.padding, this.props.schema.innerStyle) }, this.renderItems(items, disabled))));
        }
