...
class ConfigSendto extends ConfigGeneric {
    ...
    renderErrorDialog() {
        if (this.state._error) {
            return (React.createElement(DialogError, { text: this.state._error, onClose: () => this.setState({ _error: '' }) }));
        }
        return null;
    }
    renderMessageDialog() {
    ...
    renderDialogConfirm() {
        ...
        return (React.createElement(DialogConfirm, { title: this.getText(confirm.title) || I18n.t('ra_Please confirm'), text: this.getText(confirm.text), ok: this.getText(confirm.ok) || I18n.t('ra_Ok'), cancel: this.getText(confirm.cancel) || I18n.t('ra_Cancel'), icon: icon || undefined, onClose: isOk => this.setState({ confirmDialog: false }, () => isOk && this._onClick()) }));
    }
    renderItem(error, disabled) {
        const icon = this.getIcon();
        return (React.createElement("div", { style: styles.fullWidth },
            React.createElement(Button, { variant: this.props.schema.variant || undefined, color: this.props.schema.color || 'grey', style: { ...styles.fullWidth, ...(this.props.schema.controlStyle || undefined) }, disabled: disabled || !this.props.alive, startIcon: icon, title: this.props.alive
                    ? this.getText(this.props.schema.title) || ''
                    : I18n.t('ra_Instance is not alive'), onClick: () => {
