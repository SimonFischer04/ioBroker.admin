...
import { Utils, I18n, DeviceTypeIcon, } from '@iobroker/adapter-react-v5';
...
class DeviceCard extends Component {
    ...
    async fetchIcon() {
        if (!this.props.device.icon) {
            ...
            const manufacturer = this.props.device.manufacturer && typeof this.props.device.manufacturer === 'object'
                ? this.props.device.manufacturer[lang] || this.props.device.manufacturer.en
                : this.props.device.manufacturer;
            const model = this.props.device.model && typeof this.props.device.model === 'object'
    ...
    renderSmall() {
        ...
        return (React.createElement(Card, { sx: {
            ...
            React.createElement(CardHeader, { style: headerStyle, avatar: React.createElement("div", null,
                    this.props.uploadImagesToInstance ? (React.createElement(DeviceImageUpload, { uploadImagesToInstance: this.props.uploadImagesToInstance, deviceId: this.props.device.id, manufacturer: getText(this.props.device.manufacturer), model: getText(this.props.device.model), onImageSelect: (imageData) => {
                            if (imageData) {
            ...
            React.createElement(CardContent, { style: { position: 'relative' } },
                status?.length ? (React.createElement("div", { style: {
                        ...
                        borderRadius: 5,
                        width: 'calc(100% - 46px)',
                    } }, status.map((s, i) => (React.createElement(DeviceStatusComponent, { key: i, status: s, connectionType: this.props.device.connectionType, enabled: this.props.device.enabled, deviceId: this.props.device.id, statusAction: this.props.device.actions?.find(a => a.id === ACTIONS.STATUS), disableEnableAction: this.props.device.actions?.find(a => a.id === ACTIONS.ENABLE_DISABLE), deviceHandler: this.props.deviceHandler, refresh: this.refresh, theme: this.props.theme }))))) : null,
                React.createElement("div", null,
                    React.createElement(Typography, { variant: "body1" },
                        React.createElement("div", { onClick: this.copyToClipboard, style: { textOverflow: 'ellipsis', overflow: 'hidden' } },
    ...
    getCardHeaderStyle(theme, maxWidth) {
        ...
        const backgroundColor = device.backgroundColor === 'primary'
    ...
    renderBig() {
        ...
        return (React.createElement(Paper, { style: styles.cardStyle, key: this.props.id },
            React.createElement(Box, { sx: headerStyle, style: styles.headerStyle },
                React.createElement("div", { style: styles.imgAreaStyle },
                    ...
                    icon),
                React.createElement(Box, { style: styles.titleStyle, title: title.length > 20 ? title : undefined, sx: theme => ({ color: headerStyle.color || theme.palette.secondary.contrastText }) }, this.state.details?.data?.name || this.props.title),
                this.props.device.hasDetails ? (React.createElement(Fab, { disabled: !this.props.alive, size: "small", style: styles.detailsButtonStyle, onClick: () => {
                        if (!this.state.open) {
                            this.loadDetails().catch(console.error);
                            this.setState({ open: true });
                        }
            ...
            React.createElement("div", { style: styles.statusStyle }, status.map((s, i) => (React.createElement(DeviceStatusComponent, { key: i, deviceId: this.props.device.id, connectionType: this.props.device.connectionType, status: s, enabled: this.props.device.enabled, statusAction: this.props.device.actions?.find(a => a.id === ACTIONS.STATUS), disableEnableAction: this.props.device.actions?.find(a => a.id === ACTIONS.ENABLE_DISABLE), deviceHandler: this.props.deviceHandler, refresh: this.refresh, theme: this.props.theme })))),
