...
(0, checkLinuxPass_1.checkWellKnownPasswords)()
    .then(found => {
    if (found) {
        ...
        rl.question(`Enter new password for "${found.login} (min 6 chars): `, (password) => {
            ...
            rl.question(`Repeat new password for "${found.login}: `, (passwordRepeat) => {
                ...
                rl.close();
                (0, checkLinuxPass_1.setLinuxPassword)(found.login, found.password, password)
    ...
    .catch(err => {
    console.error(`Cannot check well-known passwords: ${err.toString()}`);
    process.exit(1);
});
