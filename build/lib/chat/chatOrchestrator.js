...
exports.buildSystemPromptMessage = buildSystemPromptMessage;
 ...
 */
function formatStepResult(tool, text) {
    if (OUTPUT_TOOLS.has(tool)) {
        try {
            ...
            const parts = [failed ? `✗ ${error || 'execution failed'}` : '✓ executed'];
 ...
 */
function buildSystemPromptMessage(language, mode, uiContext) {
    const lines = [
